...
static ssize_t read_file_phy_cs(struct file *file, char __user *user_buf,
       size_t count, loff_t *ppos)
{
 struct ath9k_htc_priv *sc = file->private_data;
 char buf[32];
 unsigned int len;
 int val;
        ...
        size_t count, loff_t *ppos)
{
 ...
 ath9k_hw_set_phy_cs(sc->ah, val);

 printk("-- gnychis: write_file_phy_cs: %d\n", val);
        ...
        size_t count, loff_t *ppos)
{
 ...
 val = ath9k_hw_get_virt_cs(sc->ah);
 len = snprintf(buf, sizeof(buf), "%d\n", val);
 printk("-- gnychis: read_file_virt_cs: %d\n", val);
 return simple_read_from_buffer(user_buf, count, ppos, buf, len);
}
        ...
        size_t count, loff_t *ppos)
{
 ...
 ath9k_hw_set_virt_cs(sc->ah, val);
 printk("-- gnychis: ath9k_hw_set_virt_cs: %d\n", val);
...
int ath9k_htc_init_debug(struct ath_hw *ah)
{
  ...
  printk("-- gnychis: creating debugfs\n");
	if (!priv->debug.debugfs_phy) {
    printk("-- gnychis: could not create\n");
		return -ENOMEM;
  }
  ...
  debugfs_create_file("physical_carrier_sense",
                  S_IRUSR|S_IWUSR,
                  priv->debug.debugfs_phy,
                  priv, &fops_phy_cs);

 debugfs_create_file("virtual_carrier_sense",
             S_IRUSR|S_IWUSR,
             priv->debug.debugfs_phy,
             priv, &fops_virt_cs);
